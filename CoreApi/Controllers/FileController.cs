...
namespace CoreApi.Controllers
{
    ...
    public class FileController : BaseController
    {
        ...
        /// <returns></returns>
        [HttpGet("merge")]
        public async Task<IActionResult> MergeFiles(string mergeFileName, string sliceName, string sliceType)
        {
            ...
            var sliceTypeLength = sliceType.Length + 1;
            var arr = Directory.EnumerateFiles(FilePath).Where(d => d.LastIndexOf(sliceName) > -1).OrderBy(d =>
            {
                var span = d.AsSpan(rootLength + 1).Slice(sliceNameLength);
                return int.Parse(span.Slice(0, span.Length - sliceTypeLength));
            }).ToArray();

            if (arr == null || !arr.Any()) return BadRequest("文件切片不存在,合并失败");
